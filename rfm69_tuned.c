...
int rfm69_initialize(char freqBand, char nodeID, char networkID) {
    ...
    // Data Module: Continuous mode, OOK Modulation, No Shaping
  ...
  rfm69_encrypt(0);
  	...
  	rfm69_setMode(RF_OPMODE_STANDBY);
...
void rfm69_writeReg(unsigned char addr, char value) {
...
char rfm69_readReg(unsigned char addr) {
  unsigned char thedata[2];
  thedata[0] = addr & 0x7F;
  thedata[1] = 0;
...
printf("rejestr REG_IRQFLAGS1: %x \n", rfm69_readReg(REG_IRQFLAGS1));
  // but waiting for mode ready is necessary when going from sleep because the FIFO may not be immediately available from previous mode
  while (_mode == RF_OPMODE_SLEEP && (rfm69_readReg(REG_IRQFLAGS1) & RF_IRQFLAGS1_MODEREADY) == 0x00); // Wait for ModeReady
  _mode = newMode;
